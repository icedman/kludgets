...
//            widget->setMaximumHeight(0);
    ...
    if (widget->inherits("Konsole::TerminalDisplay")
            || widget->inherits("QTextEditor::View")
...
#endif
#if 0
    if (widget->inherits("QTextEditor::View"))
    {
        QWidget *parent = widget->parentWidget();
        if (parent)
        {
    ...
    if (!qstrcmp(widget->metaObject()->className(), "QLineEditButton"))
    {
        widget->installEventFilter(d);
        widget->setAttribute(Qt::WA_Hover, true);
    }
    ...
    if (widget->inherits("QTabWidget") && widget->property("closeButtonEnabled").toBool())
    {
...
void SkulptureStyle::polish(QPalette &palette)
{
    ...
    computeColorGroups(palette, qApp->inherits("QApplication"), d->makeDisabledWidgetsTransparent);
}
...
static void sharpenRgbSpan(int count, QRgb *rgb, int offset, int f)
{
    ...
    {
        ptr += offset;
        v0 += (((ptr[0] << V_SHIFT) - v0) * f) >> F_SHIFT;
        ptr[0] = clampValue(v0);
        v1 += (((ptr[1] << V_SHIFT) - v1) * f) >> F_SHIFT;
        ptr[1] = clampValue(v1);
        v2 += (((ptr[2] << V_SHIFT) - v2) * f) >> F_SHIFT;
        ptr[2] = clampValue(v2);
        v3 += (((ptr[3] << V_SHIFT) - v3) * f) >> F_SHIFT;
        ptr[3] = clampValue(v3);
    }
    while (--count >= 0);
}
...
static void blurRgbSpan(int count, QRgb *rgb, int offset, int f)
{
    ...
    do
    {
        ptr += offset;
        v0 += (((ptr[0] << V_SHIFT) - v0) * f) >> F_SHIFT;
        ptr[0] = (v0 + V_ROUND) >> V_SHIFT;
        v1 += (((ptr[1] << V_SHIFT) - v1) * f) >> F_SHIFT;
        ptr[1] = (v1 + V_ROUND) >> V_SHIFT;
        v2 += (((ptr[2] << V_SHIFT) - v2) * f) >> F_SHIFT;
        ptr[2] = (v2 + V_ROUND) >> V_SHIFT;
        v3 += (((ptr[3] << V_SHIFT) - v3) * f) >> F_SHIFT;
        ptr[3] = (v3 + V_ROUND) >> V_SHIFT;
    }
    while (--count >= 0);
}
...
void AbstractFactory::executeCode(Code code)
{
    if (code >= SetVar + MinVar && code <= SetVar + MaxVar)
    {
        ...
        {
            const Code *loop_p = p;
            int counter = 100; // prevent inf`e loop
...
void paintStyledFrame(QPainter *painter, const QStyleOptionFrame *option, const QWidget *widget, const QStyle *)
{
...
QIcon SkulptureStyle::standardIconImplementation(QStyle::StandardPixmap standardIcon, const QStyleOption *option, const QWidget *widget) const
{
    ...
    if (code)
    {
        ...
        if (numStates == 3)
        {
            if (widget && !qstrcmp(widget->metaObject()->className(), "QLineEditButton"))
            {
...
int SkulptureStyle::pixelMetric(PixelMetric metric, const QStyleOption *option, const QWidget *widget) const
{
    ...
    case PM_ButtonMargin:
        return 2 * d->pushButtonSize;
    case PM_ButtonDefaultIndicator:
        return 0;
    case PM_MenuButtonIndicator:
        return fontHeight(option, widget);
    ...
    case PM_DefaultFrameWidth:
        return 2; // 3 for command buttons
    case PM_SpinBoxFrameWidth:
        return 2;
    case PM_ComboBoxFrameWidth:
        return 2; // 3 for non-editable combobox (in button mode)
    ...
    case PM_DockWidgetSeparatorExtent:
        return ((qMax(fontHeight(option, widget), 16) >> 1) & ~1) - 1;
    ...
    case PM_TabBarTabHSpace:
        return 2 * d->tabBarSize + (fontHeight(option, widget) & ~1);
    ...
    case PM_TitleBarHeight:
        return d->textLineHeight(option, widget) + 4;
...
#else
    case PM_MenuHMargin:
        return 0; // ### anything other than 0 messes Qt's menu positioning code ...
    case PM_MenuVMargin:
        return 0;
    case PM_MenuPanelWidth:
        return 2;
    ...
    case PM_MdiSubWindowMinimizedWidth:
        return fontHeight(option, widget) * 12;
#else
    ...
    case PM_TabBarScrollButtonWidth:
        return (fontHeight(option, widget) & ~1) + 1;
...
void paintSizeGrip(QPainter *painter, const QStyleOption *option)
{
    ...
    case Qt::TopLeftCorner:
        r = option->rect.adjusted(0, 0, 2, 2);
        break;
    case Qt::TopRightCorner:
        r = option->rect.adjusted(-2, 0, 0, 2);
        break;
    case Qt::BottomLeftCorner:
        r = option->rect.adjusted(0, -2, 2, 0);
        break;
    case Qt::BottomRightCorner:
        r = option->rect.adjusted(-2, -2, 0, 0);
        break;
    }
    paintThinFrame(painter, r, option->palette, 60, -20);
    paintThinFrame(painter, r.adjusted(1, 1, -1, -1), option->palette, -20, 60);
...
void paintScrollAreaCorner(QPainter *painter, const QStyleOption *option, const QWidget *widget, const QStyle *)
{
    QStyleOption opt;
    opt = *option;
...
void paintTabWidgetFrame(QPainter *painter, const QStyleOptionTabWidgetFrame *option, const QWidget *widget)
{
    ...
    {
    case North:
        base.translate(0, -(s - overlap));
        break;
    case West:
        base.translate(-(s - overlap), 0);
        break;
    case South:
        base.translate(0, option->rect.height() - overlap);
        break;
    case East:
        base.translate(option->rect.width() - overlap, 0);
        break;
    }
    if (s != 2)
    {
...
bool SkulptureStyle::loadSettings(QString filename)
{
    if (!QFile::exists(filename))
    {
        qDebug("file does not exits %s", qPrintable(filename));
        return false;
    }
...
void paintNothing(QPainter *, const QStyleOption *)
{
    //
}


void paintDefault(QPainter *, const QStyleOption *)
{
    //
}
